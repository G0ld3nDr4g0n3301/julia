...
fn main() {
    ...
    for j in 0..height {
        for i in 0..width {
            ...
            while x * x * y + y * y * y <= 4.0 && iterations < max_iterations {
                let temp_x = x * x * x - 3.0 * y * y * x + cx;
                y = 2.0 * x * x * y + y * x * x - y * y * y + cy;
                x = temp_x;
                iterations += 1;
            }
    ...
    img.save("julia.png").expect("Не удалось сохранить изображение");
